...
# use top 50/100 features to fit xgb
top = 100
feat_w = pd.concat([pd.DataFrame(X_train.columns, columns=['features']), 
		 pd.DataFrame(clf.feature_importances_, columns=['weights'])], axis=1).sort_values('weights', ascending=False)
clf = xgb.XGBRegressor(base_score=0.5, booster='gbtree',
		colsample_bylevel=0.3476398569954531,
		colsample_bytree=0.2731010282737144, gamma=0.4801938206633828,
		learning_rate=0.03993424498060205, max_delta_step=0, max_depth=2,
		...
		scale_pos_weight=1, seed=None, silent=True,
		subsample=0.9636596044220748)
clf.fit(X_train[feat_w.iloc[:top,0]], y)

# predict and form submission
pred = clf.predict(X_test[feat_w.iloc[:top,0]])
sub = pd.concat([test['ID'], pd.DataFrame(pred, columns=['y'])], axis=1)
sub.to_csv('../submission/sub_topfeatures.csv', index=False)
